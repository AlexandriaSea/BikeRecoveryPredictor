...
function predictRandomForest() {
  ...
  fetch("/predict/rf", {
    ...
    .then((result) => {
      // Remove placeholder if it exists
      const randomForestHistory = document.getElementById("randomForestHistory");
...
function predictDecisionTree() {
  ...
  fetch("/predict/dt", {
    ...
    .then((result) => {
      // Remove placeholder if it exists
      const decisionTreeHistory = document.getElementById("decisionTreeHistory");
      if (decisionTreeHistory.querySelector(".text-muted")) {
        decisionTreeHistory.innerHTML = ""; // Clear the placeholder
      }

      decisionTreeRunCount++;
      // Append the result to the history list
      const listItem = document.createElement("li");
      listItem.classList.add("list-group-item");
      listItem.textContent = `RUN ${decisionTreeRunCount}: ${result.decision_tree_prediction}`;
      decisionTreeHistory.appendChild(listItem);
    })
    .catch((error) => console.error("Error:", error));
}
...
function clearForm() {
  ...
  // Clear the displayed prediction results for all models
  ...
  document.getElementById("decisionTreeHistory").innerHTML =
    "<li class='list-group-item text-muted'>Prediction results for Decision Tree will appear here.</li>";
