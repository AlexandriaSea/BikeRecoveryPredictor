...
# Create the final pipeline with weighted logistic regression
...
def train_and_evaluate(X_train, X_test, y_train, y_test, target_name):
    ...
    with open('pkl/logistic_regression.pkl', 'wb') as file:
    ...
    feature_names = np.concatenate([
        numerical_columns,
        cat_encoder.get_feature_names_out(categorical_columns)
    ])
    coefficients = pipeline.named_steps['classifier'].coef_[0]

    # Create feature importance DataFrame with matched lengths
    n_features = min(len(feature_names), len(coefficients))
    importance_df = pd.DataFrame({
        'Feature': feature_names[:n_features],
        'Importance': coefficients[:n_features]
    }).sort_values(by='Importance', ascending=False)

    # Sort by absolute values to show strongest influences
    ...
    plt.tight_layout()
    plt.savefig('plot/feature_importances_log.png')
    plt.show()
    ...
    plt.savefig('plot/confusion_matrix_log.png')
    plt.show()
